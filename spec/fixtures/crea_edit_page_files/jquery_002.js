	...
	$.fn.addTag = function(v){
		...
		for(var i in r){
			...
			//var fn = $(this).data('name');
                       // alert(fn);
			var i = $('<input type="hidden" />').attr('name','motclef[]').val(n);
			var t = $('<li />').text(n).addClass('tagName');
                        t.append("&nbsp;&nbsp;<a style='color:red' href='#'>x</a>")
				.click(function(){
...
jQuery(function(){
	$('.tagger').each(function(i){
		...
		var b = $('<button type="button" />').html('Ajouter').addClass('buttons medium  marron')
			.click(function(){
